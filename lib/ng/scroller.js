...
ng.Scroller.prototype.compile = function ($viewport, attrs) {
  ...
  var $template = viewport.querySelectorAll('[ng-scroller-repeat]');
  var template_el = $template[0];

  this.template = this.$compile(template_el);
  this.carousel = template_el.parentNode;

  var exp = template_el.getAttribute('ng-scroller-repeat');
  ...
  this.carousel.removeChild(template_el);
};
...
ng.Scroller.prototype.init_ = function () {
  var styler_exp = (this.mode === 'content') ? 'ng.InPageScrollerStyler' : 'ng.ScrollerStyler';
